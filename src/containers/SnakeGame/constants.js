export const STARTED_GAME = 'STARTED_GAME';
export const INCREMENT_POSITION = 'INCREMENT_POSITION';
export const GET_KEY_CODE = 'GET_KEY_CODE';
export const RIGHT = 'RIGHT';
export const LEFT = 'LEFT';
export const UP = 'UP';
export const DOWN = 'DOWN';
export const CREATE_APPLE = 'CREATE_APPLE';
export const EAT_APPLE = 'EAT_APPLE';
export const INITIAL = 'INITIAL';
export const ADD_POINTS = 'ADD_POINTS';
export const CHANGE_DIRECTION = 'CHANGE_DIRECTION';
export const ITERATE = 'ITERATE';
export const GAME_OVER = 'GAME_OVER';
export const COLLISION_APPLE = 'COLLISION_APPLE';
export const INCREMENT_SPEED = 'INCREMENT_SPEED';
export const CHANGE_TOUCH_COORDINATES = 'CHANGE_TOUCH_COORDINATES';
export const CREATE_SNAKE = 'CREATE_SNAKE';
export const CALCUALTE_BOARD = 'CALCUALTE_BOARD';
export const INITILIZE_APPLE = 'INITILIZE_APPLE';
